...
class RoadRulesPenalty(Cost):
    def __init__(self, g_params, name="roadrules_penalty"):
        ...
        """
        # self._x_index, self._y_index = g_params["position_indices"][g_params["player_id"]] why out of index
        ...
        super(RoadRulesPenalty, self).__init__(
    ...
    def g_right_combined_withcurve(self, x, k=0):
        # first layer
        layer_1_road_rules = self.new_road_rules(
            left_lane=True, right_lane=False, up_lane=True, down_lane=True)
        # second layer
        layer_2_road_rules = self.new_road_rules(
            left_lane=True, right_lane=True, down_lane=True, up_lane=False)
    ...
    def g_road_rules(self, x, **kwargs):
        ...
        if not left_turn:
            ...
            _max_func.store(self.g_left_of_main, self.g_left_of_main(x))
            _max_func.store(self.g_circle_intersection,
                            self.g_circle_intersection(x))
            _max_func.store(self.g_right_combined_withcurve,
                            self.g_right_combined_withcurve(x))
            _func_of_max_val, _max_val = _max_func.get_max()
    ...
    def new_road_rules(self, **kwargs):
        ...
        if down_lane and not up_lane:
            new_road_rules["y_max"] = self._road_rules_original["y_max"] - \
                self._road_rules_original["width"]
        elif up_lane and not down_lane:
            new_road_rules["y_min"] = self._road_rules_original["y_min"] + \
                self._road_rules_original["width"]

        if left_lane and not right_lane:
            # Can either go straight down or turn left
            new_road_rules["x_max"] = self._road_rules_original["x_max"] - \
                self._road_rules_original["width"]
        elif right_lane and not left_lane:
            # Can either go straight up or turn right
            new_road_rules["x_min"] = self._road_rules_original["x_min"] + \
                self._road_rules_original["width"]
