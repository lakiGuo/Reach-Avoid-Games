...
class Hungarian:
    ...
    def __init__(self, input_matrix=None, is_profit_matrix=False):
        ...
        if input_matrix is not None:
            ...
            my_matrix = np.pad(my_matrix, ((0, pad_columns),
                               (0, pad_rows)), 'constant', constant_values=(0))
        ...
        while len(self._results) != expected_results:
            ...
            if not zero_locations.any():
                raise HungarianError(
                    "Unable to find results. Algorithm has failed.")
    ...
    def __mark_rows_and_columns(marked_rows, marked_columns, row_index, column_index):
        ...
        if not (marked_rows == row_index).any() and not (marked_columns == column_index).any():
            new_marked_rows = np.insert(
                marked_rows, len(marked_rows), row_index)
    ...
    def select_arbitrary_match(zero_locations):
        ...
        for index, row in enumerate(rows):
            total_zeros = np.sum(
                zero_locations[row]) + np.sum(zero_locations[:, columns[index]])
...
class CoverZeros:
    ...
    def __init__(self, matrix):
        ...
        self._covered_rows = list(
            set(range(self._shape[0])) - set(self._marked_rows))
    ...
    def __calculate(self):
        ...
        while True:
            ...
            while choice_column_index is not None:
                ...
                choice_row_index = self.__find_row_without_choice(
                    choice_column_index)
                ...
                if choice_row_index is None:
                    ...
                    choice_row_index, new_choice_column_index = \
                        self.__find_best_choice_row_and_new_column(
                            choice_column_index)

                    # Delete old choice.
                    self._choices[choice_row_index,
                                  new_choice_column_index] = False
            ...
            if index not in self._marked_columns:
                if column.any():
                    ...
                    zeros_in_marked_rows = (
                        set(self._marked_rows) & set(row_indices)) != set([])
